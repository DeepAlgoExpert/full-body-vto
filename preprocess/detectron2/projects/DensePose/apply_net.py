...
class DumpAction(InferenceAction):
    ...
    def postexecute(cls: type, context: Dict[str, Any]):
        ...
        for dic in context["results"]:
          file_name = os.path.basename(dic['file_name'])
          fname = os.path.splitext(file_name)[0]
          if(fname.endswith('_0')):
            out_fname = out_dir + '/' + fname.replace('_0','_5_uv') + '.npz'
            np.savez(out_fname, uv=dic['pred_densepose'][0].uv.cpu())
...
class ShowAction(InferenceAction):
    ...
    def execute_on_outputs(
        ...
        image_vis = visualizer.visualize(np.zeros_like(image), data)
        ...
        fname = os.path.splitext(file_name)[0]
        if(fname.endswith('_0')):
          out_fname = out_dir + '/' + fname.replace('_0','_5') + '.png'
